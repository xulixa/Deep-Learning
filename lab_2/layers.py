...
class ReLU(Layer):
  ...
  def backward_inputs(self, grads):
    ...
    Returns:
      ndarray of shape (N, C, H, W).
    """
    relu_mask = self.inputs > 0
    return grads * relu_mask
...
class SoftmaxCrossEntropyWithLogits():
  ...
  def forward(self, x, y):
    ...
    """
    softmax_x = scipy.special.softmax(x, axis=1)
    log_softmax_x = np.log(softmax_x + 1e-20)
    return np.mean(-np.sum(y * log_softmax_x, axis=1))
...
class L2Regularizer():
  ...
  def forward(self):
    ...
    """
    weights_sum = np.sum(self.weights**2)
    return 0.5 * self.weight_decay * weights_sum
